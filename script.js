$(document).ready(function () {
   ...
   var playlistData = [
    ...
    var initialSrc = document.getElementById("video-player")
    initialSrc.src = "https://player.vimeo.com/video/" + videoPlaySectionData[0].vimeoId
    $("#views-count").html(videoPlaySectionData[0].views);
    $("#video-title").html(videoPlaySectionData[0].title);
    $("#video-description").html(videoPlaySectionData[0].description);
  
  
    function createPlaylistCard(obj, vid) {
      ...
      mainDiv.addEventListener("click", function () {
        ...
        var temp = this.id - 1
        var videoId = vid[temp].vimeoId;
        var videoLink = "https://player.vimeo.com/video/" + videoId;
        var playArea = document.getElementById("video-player");
        playArea.src = videoLink;
        $("#views-count").html(vid[temp].views);
        $("#video-title").html(vid[temp].title);
        $("#video-description").html(vid[temp].description);
      })
    }
  
    for (i = 0; i < playlistData.length; i++) {
      createPlaylistCard(playlistData, videoPlaySectionData)
    }
